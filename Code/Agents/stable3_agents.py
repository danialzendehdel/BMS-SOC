...
class TensorboardCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        # Update history only if we have all required values
        if all(value is not None for value in step_values.values()):
    ...
    def get_data(self):
        ...
        min_length = min(len(arr) for arr in self.history['steps'].values())
        
        # Truncate all arrays to minimum length
        step_data = {
            key: arr[:min_length] for key, arr in self.history['steps'].items()
        }
        ...
        episode_df = pd.DataFrame(self.history['episodes'])
        ...
        def _on_step(self):
            ...
            if self.n_calls % self.eval_freq == 0:
                try:
                    ...
                    eval_env = DummyVecEnv([make_env('Code/configuration/params_2.yml', 0, seed=42)])
...
def train_and_evaluate(algo, total_timesteps, device="cpu"):
    try:
        ...
        env = setup_environment('Code/configuration/params_2.yml', journalist)
        ...
        # Evaluate the model
        try:
            eval_env = setup_environment('Code/configuration/params_2.yml', journalist)
            eval_rewards = []
            n_eval_episodes = 5
